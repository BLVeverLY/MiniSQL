...
int Interpreter::interpreter(string s)
{
	...
	if (strcmp(word.c_str(), "create") == 0)
	{
		...
		if (strcmp(word.c_str(), "table") == 0)
		{
			...
			else				// deal with attribute list
			{
				...
				ap->CreateTable(tableName,&attributeVector,primaryKey,primaryKeyLocation);
				return 1;
			}
		}
		else if (strcmp(word.c_str(), "index") == 0)
		{
			...
			try {
				...
				if (strcmp(word.c_str(),")") != 0)
					throw SyntaxException();
				ap->CreateIndex(indexName,tableName,attributeName);
				return 1;
	...
	else if(strcmp(word.c_str(), "select")==0)
	{
		...
		if (word.empty())	// without condition
		{
			if(attrSelected.size()==0){
				...
				ap->Select(tableName,&attrSelected);
			return 1;
		}
		else if (strcmp(word.c_str(),"where") == 0)
		{
			...
			while(1){
				try {
					...
					word = getWord(s,&tmp);
					if (strcmp(word.c_str(),"<=") == 0)
						operate = Condition::OP_eq_less;
					else if (strcmp(word.c_str(),">=") == 0)
						operate = Condition::OP_eq_greater;
					else if (strcmp(word.c_str(),"<") == 0)
						operate = Condition::OP_less;
					else if (strcmp(word.c_str(),">") == 0)
						operate = Condition::OP_greater;
					else if (strcmp(word.c_str(),"=") == 0)
						operate = Condition::OP_eq;
					else if (strcmp(word.c_str(),"<>") == 0)
						operate = Condition::OP_neq;
					else
						throw SyntaxException();
					word = getWord(s,&tmp);
			...
			if(attrSelected.size()==0)
				ap->Select(tableName,NULL,&conditionVector);
			else
				ap->Select(tableName, &attrSelected,&conditionVector);
	...
	else if (strcmp(word.c_str(), "delete")==0)
	{
		...
		else if (strcmp(word.c_str(),"where") == 0)
		{
			...
			ap->Delete(tableName, &conditionVector);
			return 1;
		}
	}
	...
	else if (strcmp(word.c_str(), "insert") == 0)
	{
		...
		ap->Insert(tableName,&valueVector);
		return 1;
	}
