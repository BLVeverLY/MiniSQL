...
//**********************TreeNode***************************//
//offsetNumber; // the value of the tree node
...
public:
    bool isRoot();
    bool search(KeyType key,int &index);//search a key and return by the reference of a parameter
    TreeNode* splite(KeyType &key);
    int add(KeyType &key); //add the key in the branch and return the position
    int add(KeyType &key,int val); // add a key-value in the leaf node and return the position
...
private:
    ...
    struct searchNodeParse
    {
        ...
        int index; // the position of the key
...
public:
    ...
    bool insertKey(KeyType &key,int val);
 ...
 *
 * @param KeyType
 * @param int return the position of the node by reference
 *
...
bool TreeNode<KeyType>::search(KeyType key,int &index)
{
    ...
    else
    {
        ...
        {
            int left = 0, right = count - 1, pos = 0;
            while(right>left+1)
            {
                pos = (right + left) / 2;
                if(keys[pos] == key)
...
TreeNode<KeyType>* TreeNode<KeyType>::splite(KeyType &key)
{
    ...
    if(isLeaf) // this is a leaf node
    {
        ...
        for(int i = minmumNode + 1;i < degree;i ++) // copy the right hand of the keys to the new node
        {
 ...
 */
template <class KeyType>
int TreeNode<KeyType>::add(KeyType &key)
{
    if(count == 0)
    {
        keys[0] = key;
    ...
    {
        int index = 0; // record the index of the tree
 ...
 */
template <class KeyType>
int TreeNode<KeyType>::add(KeyType &key,int val)
{
    if(!isLeaf)
    {
        cout << "Error:add(KeyType &key,int val) is a function for leaf nodes" << endl;
        return -1;
    }
    if(count == 0)
    {
    ...
    else //count > 0
    {
        ...
        if(exist)
        {
            cout << "Error:In add(Keytype &key, int val),key has already in the tree!" << endl;
            exit(3);
        }
        else // add the key into the node
        {
 ...
 *
 * @param int the position to delete
 *
 * @return bool the falg that delation successes or not
 *
 */
template <class KeyType>
bool TreeNode<KeyType>::removeAt(int index)
{
    if(index > count)
    {
        cout << "Error:In removeAt(int index), index is more than count!" << endl;
        return false;
    }
    else
    {
 ...
 */
template <class KeyType>
bool BPlusTree<KeyType>::insertKey(KeyType &key,int val)
{
    searchNodeParse snp;
    if(!root) init_tree();
